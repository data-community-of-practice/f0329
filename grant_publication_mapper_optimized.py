...
class OptimizedGrantPublicationMapper:
    ...
    def process_publication(self, publication: pd.Series, grants_df: pd.DataFrame) -> Optional[MatchResult]:
        ...
        for i, candidate in enumerate(candidates[:3]):
            print(f"    Analyzing candidate {i+1}: {candidate.grant_data['TITLE'][:50]}...")
    ...
    def run_mapping(self, grants_file: str, publications_file: str, output_file: str, sample_size: Optional[int] = None):
        ...
        for idx, (pub_idx, publication) in enumerate(publications_df.iterrows()):
            print(f"\nProcessing publication {idx+1}/{len(publications_df)}: {publication['title'][:60]}...")
...
def main():
    ...
    OUTPUT_FILE = "optimized_mapped_publications.csv"
    ...
    # Ask for sample size
    sample_input = input("Enter sample size (or press Enter for full dataset): ").strip()
    sample_size = None
    if sample_input:
        try:
            sample_size = int(sample_input)
        except ValueError:
            print("Invalid sample size, processing full dataset")
