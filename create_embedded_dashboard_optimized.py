#!/usr/bin/env python3
"""
Create an optimized embedded HTML dashboard with improved performance
- Lazy loading of publication cards
...
def create_embedded_dashboard():
    ...
    js_data = json.dumps(publications_data, ensure_ascii=False, indent=0, separators=(',', ':'))
    
    # Create the optimized HTML with embedded data
    html_content = f'''<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Publications Dashboard - BDBSF (Optimized)</title>
...
<body>
    <div id="initialLoader" class="initial-loading">
        <div>
            ...
            Loading Publications Dashboard...
    ...
    <div class="container">
        <div class="header">
            ...
            <div class="content">
                ...
                <p>Comprehensive view of research publications and grant mappings (Optimized)</p>
            </div>
        </div>
        ...
        <div class="footer">
            <div class="credits">
                <p>Powered by <span>ResearchGraph</span> • Publications Dashboard • AI-Powered Grant Mapping (Optimized)</p>
            </div>
            <div class="logo">
    ...
    <script>
        ...
        class OptimizedDashboard {{
            constructor() {{
                this.publicationsData = {js_data};
                this.allPublications = [];
                this.bdbsfPublications = [];
                this.topicAnalysis = {{}};
                this.currentTab = 'all';
                ...
                return new Promise((resolve) => {{
                    ...
                    if (window.requestIdleCallback) {{
                        window.requestIdleCallback(() => {{
                            this.allPublications = this.publicationsData;
                            this.bdbsfPublications = this.allPublications.filter(pub => 
            ...
            populateYearFilter() {{
                const years = [...new Set(this.allPublications.map(pub => pub.publication_year))];
                years.sort((a, b) => b - a);
                
                const yearFilter = document.getElementById('yearFilter');
                const existingOptions = yearFilter.querySelectorAll('option:not([value=""])');
            ...
            updateStats() {{
                document.getElementById('totalPubs').textContent = this.allPublications.length;
                document.getElementById('uniqueYears').textContent = 
                    [...new Set(this.allPublications.map(pub => pub.publication_year))].filter(y => y).length;

                document.getElementById('bdbsfTotalPubs').textContent = this.bdbsfPublications.length;
                document.getElementById('veryHighPubs').textContent = 
                    this.bdbsfPublications.filter(pub => pub['Confidence level'] === 'Very High').length;
                document.getElementById('highPubs').textContent = 
                    this.bdbsfPublications.filter(pub => pub['Confidence level'] === 'High').length;
                document.getElementById('mediumPubs').textContent = 
                    this.bdbsfPublications.filter(pub => pub['Confidence level'] === 'Medium').length;
            }}
            ...
            async applyFiltersAndDisplay() {{
                ...
                let publications = this.currentTab === 'all' ? this.allPublications : this.bdbsfPublications;
                
                // Apply filters
                this.filteredPublications = publications.filter(pub => {{
                ...
                this.updateDisplayedCount(this.filteredPublications);
                
                // Reset pagination
                this.currentPage = 0;
                this.displayedItems = [];
                
                await this.displayPublications();
            }}

            async displayPublications() {{
                const containerId = this.currentTab === 'all' ? 'allPublications' : 'bdbsfPublications';
            ...
            async loadMoreItems() {{
                ...
                const endIndex = Math.min(startIndex + this.itemsPerPage, this.filteredPublications.length);
                ...
                newItems.forEach((pub, index) => {{
                    const cardElement = this.createPublicationCardElement(pub);
                    fragment.appendChild(cardElement);
                    
                    // Add intersection observer to the last item
                    if (startIndex + index === endIndex - 1 && endIndex < this.filteredPublications.length) {{
            ...
            createPublicationCardElement(pub) {{
                const card = document.createElement('div');
                card.className = 'publication-card';
                card.innerHTML = this.createPublicationCardHTML(pub);
                return card;
            }}

            createPublicationCardHTML(pub) {{
                const confidenceClass = pub['Confidence level'] ? 
                    `confidence-${{pub['Confidence level'].toLowerCase().replace(' ', '-')}}` : '';
                
                if (this.currentTab === 'all') {{
                    return `
                        <div class="pub-title">${{this.escapeHtml(pub.title || 'Untitled')}}</div>
                        <div class="pub-authors">${{this.escapeHtml(pub.authors_list || 'No authors listed')}}</div>
                        <div class="pub-details">
                            ...
                            ${{pub.doi ? `
                                <div class="pub-detail">
                                    <strong>DOI:</strong> 
                                    <a href="https://doi.org/${{this.escapeHtml(pub.doi)}}" target="_blank">${{this.escapeHtml(pub.doi)}}</a>
                                </div>
                            ` : ''}}
                        </div>
                    `;
                    ...
                    const grantInfo = pub['Associated Grant'] ? `
                        <div class="grant-info">
                            <div class="grant-title">Associated Grant: ${{this.escapeHtml(pub['Associated Grant'] || 'N/A')}}</div>
                            <div class="grant-reasoning">${{this.escapeHtml(pub.Reasoning || 'No reasoning provided')}}</div>
                        </div>
                    ` : '';
            ...
            updateDisplayedCount(filteredPublications) {{
                if (this.currentTab === 'all') {{
            ...
            analyzeTopics() {{
                ...
                this.bdbsfPublications.forEach(pub => {{
                    if (!pub.title) return;
                    ...
                    const words = titleLower.replace(/[^a-z0-9\\s-]/g, '').split(/\\s+/).filter(word => word.length > 3 && !stopWords.has(word));
                ...
                this.topicAnalysis = topics;
                this.displayTrends();
            }}

            updateTrendsStats() {{
                const topics = Object.keys(this.topicAnalysis);
                const totalPubs = this.bdbsfPublications.length;
                const years = [...new Set(this.bdbsfPublications.map(pub => pub.publication_year).filter(y => y))].sort();
                const avgPubsPerYear = years.length > 0 ? Math.round(totalPubs / years.length) : 0;
                
                let topTopic = 'N/A';
                let maxCount = 0;
                Object.keys(this.topicAnalysis).forEach(topic => {{
                    if (this.topicAnalysis[topic].count > maxCount) {{
                        maxCount = this.topicAnalysis[topic].count;
            ...
            displayTrends() {{
                const topicTrendsHtml = Object.keys(this.topicAnalysis)
                    .sort((a, b) => this.topicAnalysis[b].count - this.topicAnalysis[a].count)
                    .slice(0, 10)
                    .map(topic => {{
                        const data = this.topicAnalysis[topic];
                        ...
                        return `
                            <div class="topic-card">
                                <div class="topic-name">${{this.escapeHtml(topic)}}</div>
                ...
                const yearCounts = {{}};
                years.forEach(year => {{
                    yearCounts[year] = this.bdbsfPublications.filter(pub => pub.publication_year === year).length;
                }});
        ...
        window.addEventListener('load', () => {{
            dashboardInstance = new OptimizedDashboard();
        }});

        function showTab(tabName) {{
            if (dashboardInstance) {{
                dashboardInstance.showTab(tabName);
            }}
        }}

        function clearFilters() {{
            if (dashboardInstance) {{
                dashboardInstance.clearFilters();
            }}
        }}
    </script>
</body>
    ...
    with open('publications_dashboard_optimized.html', 'w', encoding='utf-8') as f:
        f.write(html_content)
    
    print("Created publications_dashboard_optimized.html with performance enhancements")
    ...
    print("- Progressive loading with visual progress bar")
    print("- Virtual scrolling with lazy loading (20 items per page)")
    print("- Debounced search (300ms delay)")
    print("- Non-blocking data processing with requestIdleCallback")
    print("- Intersection Observer for infinite scroll")
    print("- Minified JSON data")
    print("- Document fragments for efficient DOM manipulation")
