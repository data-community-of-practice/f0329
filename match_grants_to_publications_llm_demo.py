...
# Demo Configuration
DEMO_MODE = True
TEST_LIMIT = 100  # Limit to first 100 publications for demo
...
try:
    config = load_config()
    api_key_from_config = config.get('openai', 'api_key', fallback=None)
except:
...
LLM_CONFIG = {
    "provider": "openai",
    "model": "gpt-4o-mini",
    "api_key": api_key_from_config or os.getenv("OPENAI_API_KEY"),
...
def setup_llm_client():
    ...
    if not api_key:
        print("Error: No API key found. Please set api_key in config.ini file or OPENAI_API_KEY environment variable.")
        return None
    print(f"Using API key from config file: {api_key[:10]}...")
...
def main():
    ...
    try:
        ...
        print(f"[OK] Loaded {len(grants_df)} grants from Excel file")
        ...
        print(f"[OK] Loaded {len(publications_df)} publications from CSV file")
    ...
    print(f"[OK] Processed {len(processed_grants)} valid grants with investigators")
    ...
    output_filename = f"Mapped grants with publications - LLM Based_{timestamp}.csv"
